...
use crate::v1::{
    simulation::{portfolio_value, run_simulation_range},
    structs::{SimConfigWithAssumptions, Vec6, Vec6Ext},
};
...
pub fn run_percentile_monte_carlo(
    ...
    let config = SimConfigWithAssumptions::new(alloc, portfolio);
    ...
    for step in 0..config.n_steps {
        ...
        let mut values: Vec<f64> = price_map.iter().map(|p| portfolio_value(p)).collect();
...
pub fn step_averaged_monte_carlo(
    ...
    return price_map.iter().map(|p| portfolio_value(p)).sum::<f64>() / (price_map.len() as f64);
}
...
pub fn run_combined_monte_carlo(
    ...
    for step in 0..config.n_steps {
        ...
        let port_values: Vec<f64> = price_map.iter().map(|p| portfolio_value(p)).collect();
